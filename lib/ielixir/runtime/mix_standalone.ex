defmodule IElixir.Runtime.MixStandalone do
  defstruct [:node, :server_pid, :project_path]

  # A runtime backed by a standalone Elixir node managed by IElixir.
  #
  # This runtime is similar to `IElixir.Runtime.ElixirStandalone`,
  ...
  * `{:error, message}` - a final message indicating failure
  ...
  def init(project_path) do
    %{ref: ref} = emitter = IElixir.Utils.Emitter.new(self())
...
defimpl IElixir.Runtime, for: IElixir.Runtime.MixStandalone do
  ...
  def duplicate(runtime) do
    IElixir.Runtime.MixStandalone.init(runtime.project_path)
  end
end
